...
from model.i3dshufflenet.i3dshufflenet_dann import I3D_Shufflenet
...
if __name__ == '__main__':
    ...
    path_name = 'i3d_shufflenet_dann_' + source_str + source_dataset + '_' + flow_str + '.pt'
    ...
    if FLOW_TRAINING:
        ...
        checkpoint_save_path = 'model/i3d-shufflenet/'+path_name
    ...
    exp_name = 'i3d_shufflenet_exp_' + str(len(glob.glob('runs/**/*.0', recursive=True)))
    prefix = 'runs/'
    ...
    if FLOW_TRAINING:
        model = nn.DataParallel(I3D_Shufflenet(num_class=12).to(device=DEVICE))
        domain_classifier = nn.DataParallel(DomainClassifier(in_dim=12544, hidden_dim=512).to(device=DEVICE))
    else:
        model = nn.DataParallel(I3D_Shufflenet(num_class=12).to(device=DEVICE))
        domain_classifier = nn.DataParallel(DomainClassifier(in_dim=12544, hidden_dim=512).to(device=DEVICE))
    model.train()
