...
from model.i3d.i3d_dann import InceptionI3d
...
if __name__ == '__main__':
    ...
    path_name = 'i3d_dann_' + source_str + source_dataset + '_' + flow_str + '.pt'
    ...
    if FLOW_TRAINING:
        ...
        checkpoint_save_path = 'model/i3d/'+path_name
    ...
    if FLOW_TRAINING:
        model = nn.DataParallel(InceptionI3d(num_class=12).to(device=DEVICE))
        domain_classifier = nn.DataParallel(DomainClassifier(in_dim=256, hidden_dim=64).to(device=DEVICE))
    else:
        model = nn.DataParallel(InceptionI3d(num_class=12).to(device=DEVICE))
        domain_classifier = nn.DataParallel(DomainClassifier(in_dim=256, hidden_dim=64).to(device=DEVICE))
    model.train()
    ...
    for epoch in range(EPOCHS):
        ...
        for i in progressbar.progressbar(range(data_length)):
            ...
            if source_only:
                ...
                # Classification loss
                loss = domain_loss_weight * target_domain_loss + source_domain_loss + source_classification_loss
