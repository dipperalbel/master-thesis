...
from model.i3dshufflenet.i3dshufflenet import I3D_Shufflenet
...
if __name__ == '__main__':
    ...
    if FLOW_TRAINING:
        ...
        checkpoint_save_path = 'model/i3dshufflenet/flow_checkpoint_whole.pt'
        ...
        checkpoint_save_path = 'model/i3dshufflenet/rgb_checkpoint_whole.pt'
    ...
    exp_name = 'i3dshufflenet_exp_' + str(len(glob.glob('runs/**/*.0', recursive=True)))
    prefix = 'runs/'
    ...
    if FLOW_TRAINING:
        model = nn.DataParallel(I3D_Shufflenet().to(device=DEVICE))
    else:
        model = nn.DataParallel(I3D_Shufflenet().to(device=DEVICE))
    model.train()
    ...
    for epoch in range(EPOCHS):
        ...
        if NORM_ACCURACY:
            ...
            conf_mat_fig = plot_conf_mat(target=target, pred=pred_name, labels=database_train.classes, weights=weight)
        else:
            accuracy = accuracy_score(target, pred_name)
            conf_mat_fig = plot_conf_mat(target=target, pred=pred_name, labels=database_train.classes)
