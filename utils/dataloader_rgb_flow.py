...
class action_dataset:
    ...
    def __init__(self, T, end_paths, small=False, T_eval=None):
        ...
        self.video_list = []
        for end_path in self.end_path:
            for class_name in self.classes:
                self.videos = os.listdir('flow_dataset/' + class_name + end_path)
                for video in self.videos:
                    frames = len(glob.glob('flow_dataset/'+ class_name + end_path + '/' + video + '/img_*.jpg'))
                    video_class = None
                    int_class = None
                    ...
                    if frames > self.T_min:
                        ...
                        if video_class is not None:
                            ...
                            self.t_video_dict = {'class name': video_class, 'class int': int_class,
                                                'video path': 'flow_dataset/' + class_name + end_path + '/' + video,
                                                'rgb path': 'dataset/' + class_name + end_path + '/' + video + '.avi',
    ...
    def get_flow(self, idx):
        ...
        return image, frame_real

    def get_rgb(self, idx):
        size = 226.
        # Start the capture of the video
        cap = cv.VideoCapture(self.video_list[idx]['rgb path'])
    ...
    def __getitem__(self, idx):
        ...
        image_flow, frame_real = self.get_flow(idx)
        image_rgb = self.get_rgb(idx)

        image_transformed_flow = image_flow.copy()
        image_transformed_rgb = image_rgb.copy()

        class_name = self.video_list[idx]['class name']
        class_int = self.video_list[idx]['class int']
        if not self.evaluate:
            # Transform the video randomly
            image_transformed_rgb, image_transformed_flow = self.transform(image_rgb, image_flow)
            if self.video_list[idx]['class name'] == 'sit':
                reverse_chance = np.random.random()
                if reverse_chance > 0.5:
                    image_transformed_rgb = np.flip(image_transformed_rgb, axis=0)
                    image_transformed_flow = np.flip(image_transformed_flow, axis=0)
                    class_name = 'stand'
        ...
        image_transformed_rgb = self.pad_remaining_frames(image_transformed_rgb)
        image_transformed_flow = self.pad_remaining_frames(image_transformed_flow)
        # Return as a dictionary
        sample = {'rgb video': image_transformed_rgb.copy(),
                  'flow video': image_transformed_flow.copy(),
                  'class': class_name,
                  'integer class': class_int,
                  'real frame': frame_real.copy()}
        return sample

    def transform(self, image_0, image_1):
        """
