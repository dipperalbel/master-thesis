...
class action_dataset:
    ...
    def __init__(self, T, end_paths, small=False, T_eval=None, dataset='hmdb-51', data_type='rgb'):
        """
        ...
        path = 'dann_datasets/'+data_type+'/'+dataset
        ...
        for path in self.end_path:
            for j, class_name in enumerate(self.classes):
                self.videos = os.listdir('dann_datasets/' + data_type + '/' + dataset + '/' + class_name + path)
                for video in self.videos:
                    frames = len(glob.glob('dann_datasets/' + data_type + '/' + dataset + '/' + class_name + path + '/' + video + '/frame*.jpg'))
                    video_class = None
                    ...
                    if frames > self.T_min:
                        ...
                        self.t_video_dict = {'class name': self.converter[class_name], 'class int': int_class,
                                            'video path': 'dann_datasets/' + data_type + '/' + dataset + '/' + class_name + path +'/'+ video,
    ...
    def __getitem__(self, idx):
        ...
        image_sequence = []
        real_sequence = []
        for image_rgb in image_list_rgb:
            frame_rgb = cv.imread(image_rgb)
            frame_real = frame_rgb.copy()
            # cv.imshow('teste', frame_rgb)
            # cv.waitKey(24)
            
            total_frame = frame_rgb
